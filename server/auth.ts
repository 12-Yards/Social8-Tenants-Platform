...
export function registerAuthRoutes(app: Express) {
  ...
  app.post("/api/auth/signup", async (req, res) => {
    try {
      ...
      const existingUser = await prisma.users.findFirst({ where: { email: email.toLowerCase(), tenantId } });
      if (existingUser) {
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      const tenantId = (req as any).tenantId || null;
      const user = await prisma.users.findFirst({ where: { email: email.toLowerCase(), tenantId } });
  ...
  app.get("/api/auth/sso", async (req, res) => {
    try {
      ...
      const ssoTenantId = (req as any).tenantId || null;
      let existingUser = await prisma.users.findFirst({ where: { email: emailLower, tenantId: ssoTenantId } });
  ...
  app.post("/api/admin/users", adminOnly, async (req: any, res) => {
    try {
      ...
      const adminTenantId = req.tenantId || null;
      const existingUser = await prisma.users.findFirst({ where: { email: email.toLowerCase(), tenantId: adminTenantId } });
      if (existingUser) {
        return res.status(400).json({ error: "A user with this email already exists" });
      }
      
      const existingName = await prisma.users.findFirst({ where: { mumblesVibeName, tenantId: adminTenantId } });
      if (existingName) {
